...
def greedy_select_nouse(embeddings, k, i):
    return
    ...
    #         # selected_indices.append(min_row)
    #         # selected_indices.append(min_col)
    #         df.iloc[min_row, min_col] = np.inf
...
def select_represent_sample(model, data_path, embedding_store_path, sample_store_path, category_labels, n_sample, top_k):
    ...
    def all_data():
        ...
        print("total: "+str(len(texts))+", "+str(mean_cosine))
    ...
    def sum_by_row(embeddings, k, i):
        ...
        # print(top_k_df)
        write_path = r"DataPipeline/output/dialog/table_new/p1_corr_Cos.csv"
    ...
    def greedy_select(embeddings, k, i):
        ...
        selected_indices = []
        while len(set(selected_indices)) < k:
            min_value = torch.min(cosine)
            min_indices = torch.argmin(cosine).item()
            min_row = min_indices // cosine.shape[1]
            min_col = min_indices % cosine.shape[1]
            # cosine[min_row, min_col] = float('inf') # 将最小值设为无穷大 compare2
            cosine[min_row, :] = float('inf')  # 将最小值所在的行设为无穷大 compare1
            cosine[:, min_col] = float('inf')  # 将最小值所在的列设为无穷大
            if min_row in selected_indices and min_col in selected_indices:
                continue
            if min_row not in selected_indices:
                selected_indices.append(min_row)
            if min_col not in selected_indices:
                selected_indices.append(min_col)
            # print(len(selected_indices))
        # print(selected_indices)
        top_k_df = df_dataset[df_dataset['riskType'] == category_labels[i]].iloc[selected_indices]

        write_path = r"DataPipeline/output/dialog/table_new/p2_corr_Cos.csv"
        if i==0:
            top_k_df.to_csv(write_path,index=False,encoding="utf-8",mode='a')
        else:
            top_k_df.to_csv(write_path,index=False,encoding="utf-8",mode='a',header=False)
    ...
    def groupby_data():
        ...
        for i, docs in enumerate(dataset):  # 分类
            ...
            embeddings = torch.tensor(embeddings).to(torch.float32)
            mean_cosine = compute_mean_cosine_similarity(embeddings)
            print(category_labels[i]+": "+str(len(texts))+", "+str(mean_cosine))
...
def main(args):
    ...
    category_labels = [ '冒充电商物流客服类', '冒充领导、熟人类', '虚假信用服务类', '冒充公检法及政府机关类',
                        '虚假网络投资理财类', '网络婚恋、交友类', '冒充军警购物类诈骗',
                        '网黑案件', '虚假购物、服务类', '无风险'] # 未完成
