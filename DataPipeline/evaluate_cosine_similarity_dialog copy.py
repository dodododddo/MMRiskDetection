...
def select_represent_sample(model, data_path, embedding_store_path, sample_store_path, category_labels, n_sample, top_k):
    ...
    #         df.iloc[min_indices[0], min_indices[1]] = np.inf
    ...
    def greedy_select(embeddings, k, i):
        ...
        selected_indices = []
        while len(set(selected_indices)) < k:
            min_value = torch.min(cosine[torch.triu(torch.ones(cosine.size()), diagonal=1) == 1])
            min_indices = torch.nonzero(cosine == min_value)[0]
            cosine[min_indices[0], min_indices[1]] = float('inf')
            cosine[min_indices[1], min_indices[0]] = float('inf')
            if min_indices[0].item() in selected_indices and min_indices[1].item() in selected_indices:
                continue
            if min_indices[0].item() not in selected_indices:
                selected_indices.append(min_indices[0].item())
            if min_indices[1].item() not in selected_indices:
                selected_indices.append(min_indices[1].item())
            print(len(selected_indices))
        print(selected_indices)
        top_k_df = df_dataset[df_dataset['riskType'] == category_labels[i]].iloc[selected_indices]
        top_k_df.to_csv("DataPipeline/output/message/keyword_message_small.csv", index=False, encoding="utf-8", mode='a')
        return top_k_df
...
def main(args):
    ...
    category_labels = [ '虚假网络投资理财类', 
                        '网络婚恋、交友类', '冒充军警购物类诈骗', '网黑案件', '虚假购物、服务类', '无风险',
                        '冒充电商物流客服类', '冒充领导、熟人类',
                        '虚假信用服务类', '冒充公检法及政府机关类'] # 未完成
    # '''
...
def parse_arguments():
    ...
    parser.add_argument('--data_path', type=str, default='./DataPipeline/output/message/keyword_message_mul.csv',
