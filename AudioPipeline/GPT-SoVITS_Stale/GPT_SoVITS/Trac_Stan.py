...
from module.mel_processing import spectrogram_torch  # Import the spectrogram_torch function
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut="不切", top_k=20, top_p=0.6, temperature=0.6, ref_free=False):
    ...
    if (text[0] not in splits and len(get_first(text)) < 4): text = "。" + text if text_language != "en" else "."
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_language", type=str, default="中文", choices=["中文", "英文", "日文", "中英混合", "日英混合", "多语种混合"], help="Language of the prompt.")
    ...
    sampling_rate, audio = get_tts_wav(
        ...
        prompt_language=args.prompt_language,        text=args.text,
    ...
    sf.write(output_path, audio.astype(np.float32) / 32768, sampling_rate)  # Use soundfile.write instead
